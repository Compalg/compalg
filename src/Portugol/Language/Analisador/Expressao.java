   ...
   //--------------------------------------------------------------------------- 
    ...
    public static int TypeExpression(String elem,Vector memory){
        try{
            ...
            if(Values.IsBoolean(result) )   return Simbolo.LOGICO;
            if(Values.IsInteger(result) )   return Simbolo.INTEIRO;
            if(Values.IsCharacter(result))  return Simbolo.CARACTER;
            if(Values.IsString(result) )    return Simbolo.TEXTO;
            if(Values.IsReal(result) )      return Simbolo.REAL;
